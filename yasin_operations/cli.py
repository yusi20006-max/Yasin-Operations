...
from yasin_operations.config.config import InvalidConfigurationError, OperationsConfig, load_config
...
def build_runtime(config_overrides: dict[str, object] | None = None):
...
def _add_global_options(parser: argparse.ArgumentParser, *, suppress_defaults: bool = False) -> None:
    ...
    parser.add_argument("--log-level", choices=sorted(OperationsConfig._VALID_LOG_LEVELS), default=default, help="override log level")
    always_on = parser.add_mutually_exclusive_group()
    always_on.add_argument("--always-on", dest="always_on", action="store_true", default=default, help="enable always-on behavior")
    always_on.add_argument("--no-always-on", dest="always_on", action="store_false", default=default, help="disable always-on behavior")
...
def _configuration_overrides(args: argparse.Namespace) -> dict[str, object]:
    ...
    if services is not None:
        overrides["service_names"] = tuple(sorted({name.strip() for name in services.split(",") if name.strip()}))
    return overrides


def _operation_for_command(command: str, service: str) -> Operation:
    return Operation(name=f"service_{command}", target=OperationTarget(kind="service", identifier=service), safety_class=SafetyClass.MUTATING)
...
def _service_summary(services: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    if counts["failed"] or counts["unknown"] or counts["other"]:
        health, exit_code = "unhealthy", STATUS_EXIT_ERROR
    elif counts["degraded"] or counts["stopped"] or counts["running"] != total:
        health, exit_code = "degraded", STATUS_EXIT_DEGRADED
    else:
        health, exit_code = "healthy", STATUS_EXIT_OK
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if command == "doctor":
        diagnostics = detect_termux(config.service_root, sv_path=config.sv_path, expected_services=config.service_names)
        success = not diagnostics.issues
        result = {
            "schema_version": SCHEMA_VERSION, "command": command, "success": success, "termux": diagnostics.as_dict(), "resources": resource_snapshot().as_dict(),
            "configuration": {"service_root": config.service_root, "sv_path": config.sv_path, "service_names": list(config.service_names), "always_on": config.always_on, "execution_timeout_seconds": config.execution_timeout_seconds, "startup_grace_seconds": config.startup_grace_seconds, "log_level": config.log_level, "missing_services": list(config.missing_services())},
    ...
    if command == "status":
        result = executor.execute(Operation(name="list_services", target=OperationTarget(kind="service", identifier="*"), safety_class=SafetyClass.READ_ONLY), actor="cli", source="cli.status")
        data = dict(result.data or {}); services = list(data.get("services", [])); summary = _service_summary(services)
        _emit({"schema_version": SCHEMA_VERSION, "command": command, "success": result.success, "data": {"services": services, "summary": summary}, "error": _error(result)}, as_json)
        return STATUS_EXIT_ERROR if not result.success else int(summary["exit_code"])
    if command == "health":
        health_result = executor.execute(Operation(name="health_check", target=OperationTarget(kind="self", identifier="runtime"), safety_class=SafetyClass.READ_ONLY), actor="cli", source="cli.health")
        service_result = executor.execute(Operation(name="list_services", target=OperationTarget(kind="service", identifier="*"), safety_class=SafetyClass.READ_ONLY), actor="cli", source="cli.health.services")
        health = dict(health_result.data or {}); service_data = dict(service_result.data or {}); services = list(service_data.get("services", [])); service_summary = _service_summary(services); success = health_result.success and service_result.success
        _emit({"schema_version": SCHEMA_VERSION, "command": command, "success": success, "health": health, "services": {"items": services, "summary": service_summary}, "resources": resource_snapshot().as_dict(), "error": _error(health_result) or _error(service_result)}, as_json)
        if not success:
            return HEALTH_EXIT_UNHEALTHY
...
def _error(result: Any) -> dict[str, Any] | None:
    if result.error is None:
        return None
    return {"category": result.error.category.value, "message": result.error.message, "details": dict(result.error.details)}
