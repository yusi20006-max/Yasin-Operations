...
fixed runit argv tuples; no shell strings or arbitrary command execution are
exposed through the adapter API.
"""
...
class RunitServiceBackend(ServiceBackend):
    ...
    def __init__(self, inspector: ProcessInspector, service_root: str = "/data/data/com.termux/files/usr/var/service", definitions: Optional[Sequence[RunitServiceDefinition]] = None, timeout: float = 10.0) -> None:
    ...
    def available(self) -> bool:
        return self._root.is_dir() and (self._root.parent / "sv").exists() or Path("/data/data/com.termux/files/usr/bin/sv").exists()
    ...
    def get_status(self, name: str) -> ServiceInfo:
        ...
        if d.process_pattern:
            matches = self._inspector.find_by_name(d.process_pattern)
            if matches:
                pid = min(p.pid for p in matches)
    ...
    def _sv(self, d: RunitServiceDefinition, action: str) -> subprocess.CompletedProcess[str]:
        ...
        if not Path(sv).exists():
            sv = "sv"
        return subprocess.run([sv, action, service_dir], capture_output=True, text=True, timeout=self._timeout, check=False)
