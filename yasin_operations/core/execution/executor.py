...
class ExecutionLimits:
    """Bound untrusted operation payloads and returned result data."""
...
class Executor:
    ...
    _RETRYABLE_ERRORS = frozenset({
        ...
        ErrorCategory.UNAVAILABLE_DEPENDENCY,
    ...
    def execute(
        ...
        *,
        ...
        for attempt in range(1, attempts + 1):
            ...
            # Cooperative backoff is intentionally tiny and bounded; the next
            # attempt re-checks cancellation and the overall execution budget.
    ...
    def _walk(self, value: Any, depth: int, count: list[int]) -> None:
        if depth > self.limits.max_parameter_depth:
            raise ValueError("parameter nesting exceeds executor depth limit")
        count[0] += 1
        if count[0] > self.limits.max_parameter_items:
            raise ValueError("parameter item count exceeds executor limit")
    ...
    def _effective_attempts(max_attempts: int, capability: ToolCapability, safety_class: SafetyClass) -> tuple[int, str | None]:
        if max_attempts <= 1:
            return 1, None
        if not capability.retryable:
            return 1, "retry_disabled_by_tool_capability"
        if safety_class is SafetyClass.MUTATING and not capability.idempotent:
            return 1, "mutation_not_declared_idempotent"
    ...
    def _can_retry(self, result: OperationResult, capability: ToolCapability, safety_class: SafetyClass, attempt: int, attempts: int) -> bool:
        if attempt >= attempts or not capability.retryable or result.success or result.error is None:
            return False
        if result.error.category not in self._RETRYABLE_ERRORS:
            return False
        if result.error.category is ErrorCategory.AMBIGUOUS_OUTCOME:
            return False
